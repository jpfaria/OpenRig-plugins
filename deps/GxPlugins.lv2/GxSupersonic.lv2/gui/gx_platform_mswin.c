...
void gx_gui_create_window_and_surface(gx_supersonicUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_supersonicUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_supersonicUI *ui) {
	...
	UnregisterClass(TEXT("gx_supersonicUIClass"), NULL);
}
...
void event_handler(gx_supersonicUI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_supersonicUI *ui = (gx_supersonicUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
