...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_supersonicUI * const ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_supersonicUI* ui = (gx_supersonicUI*)malloc(sizeof(gx_supersonicUI));
	...
	ui->controls[1] = (gx_controller) {{0.15, 0.15, 0.0, 1.0, 0.01}, {300, 60, 81, 81}, false,"GAIN", KNOB, GAIN};
	ui->controls[2] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {400, 60, 81, 81}, false,"BASS", KNOB, BASS};
	ui->controls[3] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {500, 60, 81, 81}, false,"TREBLE", KNOB, TREBLE};
	ui->controls[4] = (gx_controller) {{0.25, 0.25, 0.0, 1.0, 0.01}, {600, 60, 81, 81}, false,"VOLUME", KNOB, VOLUME};
	ui->block_event = -1;
	ui->start_value = 0.0;
	ui->sc = NULL;
	ui->set_sc = 0;
...
static void cleanup(LV2UI_Handle handle) {
	gx_supersonicUI * const ui = (gx_supersonicUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(const gx_supersonicUI * const ui, const gx_controller * const knob) {
...
static void draw_grid(const gx_supersonicUI * const ui) {

	const double x0 = 130.0;
	const double y0 = 58.0;
	const double x1 = 290.0;
	const double y1 = 140.0;

	cairo_pattern_t* pat = cairo_pattern_create_radial (270, 140,
											  1,270,140,140 );
...
static void bypass_expose(const gx_supersonicUI * const ui,  const gx_controller * const switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(const gx_supersonicUI * const ui,  const gx_controller * const controller) {
	if (controller->type < BSWITCH) knob_expose(ui, controller);
	else bypass_expose(ui, controller);
}

// general XWindow expose callback, 
void _expose(const gx_supersonicUI * const ui) {
...
void controller_expose(const gx_supersonicUI * const ui,  const gx_controller * const control) {
	cairo_push_group (ui->cr);
...
void resize_event(gx_supersonicUI * const ui) {
...
static void send_controller_event(gx_supersonicUI * const ui, int controller) {
...
static void check_value_changed(gx_supersonicUI * const ui, int i, const float * const value) {
...
static void check_is_active(gx_supersonicUI * const ui, int i, bool set) {
...
static bool aligned(int x, int y, const gx_controller * const control, const gx_supersonicUI * const ui) {
...
bool get_active_ctl_num(gx_supersonicUI * const ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num( const gx_supersonicUI * const ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
void scroll_event(gx_supersonicUI * const ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_supersonicUI * const ui, int i) {
	float value;
...
void motion_event(gx_supersonicUI * const ui, double start_value, int m_y) {
...
void set_key_value(gx_supersonicUI * const ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void key_event(gx_supersonicUI * const ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
void set_previous_controller_active(gx_supersonicUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void set_next_controller_active(gx_supersonicUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_supersonicUI * const ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
