...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_sloopyblueUI * const ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_sloopyblueUI* ui = (gx_sloopyblueUI*)malloc(sizeof(gx_sloopyblueUI));
	...
	ui->controls[1] = (gx_controller) {{0.25, 0.25, 0.0, 1.0, 0.01}, {220, 55, 81, 81}, false,"GAIN", KNOB, GAIN};
	ui->controls[2] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {380, 55, 81, 81}, false,"TONE", KNOB, TONE};
	ui->controls[3] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {540, 55, 81, 81}, false,"VOLUME", KNOB, VOLUME};
	ui->block_event = -1;
	ui->start_value = 0.0;
	ui->sc = NULL;
	ui->set_sc = 0;
...
static void cleanup(LV2UI_Handle handle) {
	gx_sloopyblueUI * const ui = (gx_sloopyblueUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(const gx_sloopyblueUI * const ui, const gx_controller * const knob) {
...
static void bypass_expose(const gx_sloopyblueUI * const ui,  const gx_controller * const switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(const gx_sloopyblueUI * const ui,  const gx_controller * const controller) {
	if (controller->type < BSWITCH) knob_expose(ui, controller);
	else bypass_expose(ui, controller);
}

// general XWindow expose callback, 
void _expose(const gx_sloopyblueUI * const ui) {
...
void controller_expose(const gx_sloopyblueUI * const ui,  const gx_controller * const control) {
	cairo_push_group (ui->cr);
...
void resize_event(gx_sloopyblueUI * const ui) {
...
static void send_controller_event(gx_sloopyblueUI * const ui, int controller) {
...
static void check_value_changed(gx_sloopyblueUI * const ui, int i, const float * const value) {
...
static void check_is_active(gx_sloopyblueUI * const ui, int i, bool set) {
...
static bool aligned(int x, int y, const gx_controller * const control, const gx_sloopyblueUI * const ui) {
...
bool get_active_ctl_num(gx_sloopyblueUI * const ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num( const gx_sloopyblueUI * const ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
void scroll_event(gx_sloopyblueUI * const ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_sloopyblueUI * const ui, int i) {
	float value;
...
void motion_event(gx_sloopyblueUI * const ui, double start_value, int m_y) {
...
void set_key_value(gx_sloopyblueUI * const ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void key_event(gx_sloopyblueUI * const ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
void set_previous_controller_active(gx_sloopyblueUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void set_next_controller_active(gx_sloopyblueUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_sloopyblueUI * const ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
