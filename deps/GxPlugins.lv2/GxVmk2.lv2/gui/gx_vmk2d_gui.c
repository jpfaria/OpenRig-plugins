...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_vmk2dUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	ui->width = ui->init_width -40 + (20 * CONTROLS);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/(ui->init_width-40 + (20 * CONTROLS));
...
static void knob_expose(gx_vmk2dUI *ui,gx_controller* knob) {
	...
	cairo_pattern_add_color_stop_rgb (pat, 0.5, 0.6, 0.6, 0.6);
	cairo_pattern_add_color_stop_rgb (pat, 0.7, 0.84, 0.84, 0.84);
	cairo_pattern_add_color_stop_rgb (pat, 0.9, 0.42, 0.42, 0.42);
	cairo_pattern_add_color_stop_rgb (pat, 1, 0.42, 0.42, 0.42);
	...
	cairo_set_line_width(ui->crf,min(5, max(4,knob_x/30)));
    cairo_set_source_rgb (ui->crf,0.1,0.1,0.1);
	cairo_stroke_preserve(ui->crf);
	cairo_move_to(ui->crf, radius_x, radius_y);
	cairo_line_to(ui->crf,lengh_x,lengh_y);
	cairo_set_line_width(ui->crf,min(4, max(3,knob_x/30)));
    cairo_set_source_rgb (ui->crf,0.6,0.0,0.0);
	cairo_stroke(ui->crf);
	cairo_new_path (ui->crf);
	...
	if (knob->type == KNOB && knob->is_active == true) {
		...
		cairo_set_source_rgba (ui->crf, 0.3, 0.3, 0.3,0.6);
...
static void bypass_expose(gx_vmk2dUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_vmk2dUI *ui, gx_controller* controller) {
...
void _expose(gx_vmk2dUI *ui) {
	static const char* plug_name = "GxVmk2" ;
	...
	cairo_scale (ui->cr, 1.74, 2.95);
	cairo_set_source_surface (ui->cr, ui->frame1, 11, 12);
	cairo_paint (ui->cr);
	cairo_set_source_surface (ui->cr, ui->frame1, 112, 12);
	cairo_paint (ui->cr);
	cairo_scale (ui->cr, 1/1.74, 1/2.95);
	cairo_scale (ui->cr, 1.15, 2.95);
	cairo_set_source_surface (ui->cr, ui->frame1, 323, 12);
	cairo_paint (ui->cr);
	cairo_set_source_surface (ui->cr, ui->frame1, 425, 12);
	cairo_paint (ui->cr);
	cairo_scale (ui->cr, 1/1.15, 1/2.95);
	cairo_scale (ui->cr, 0.95, 2.95);
	cairo_set_source_surface (ui->cr, ui->frame1, 640, 12);
	cairo_paint (ui->cr);
	cairo_scale (ui->cr, 1/0.95, 1/2.95);
	...
	cairo_move_to (ui->cr, 760.0*ui->rescale.x2, 60.0*ui->rescale.y2);
...
void controller_expose(gx_vmk2dUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
void resize_event(gx_vmk2dUI *ui) {
...
static void send_controller_event(gx_vmk2dUI *ui, int controller) {
...
static void check_value_changed(gx_vmk2dUI *ui, int i, float* value) {
...
static void check_is_active(gx_vmk2dUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_vmk2dUI *ui) {
...
bool get_active_ctl_num(gx_vmk2dUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_vmk2dUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
void scroll_event(gx_vmk2dUI *ui, int direction) {
	float value;
	int num;
...
void button1_event(gx_vmk2dUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
		if (ui->controls[num].type == SWITCH) {
			switch_event(ui, num);
...
void motion_event(gx_vmk2dUI *ui, double start_value, int m_y) {
	static const double scaling = 0.5;
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
		if (ui->controls[num].type != ENUM && ui->controls[num].type != SWITCH) {
...
void set_key_value(gx_vmk2dUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void key_event(gx_vmk2dUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
void set_previous_controller_active(gx_vmk2dUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void set_next_controller_active(gx_vmk2dUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_vmk2dUI *ui, bool set) {
