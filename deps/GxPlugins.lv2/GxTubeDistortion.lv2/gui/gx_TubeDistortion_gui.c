...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_TubeDistortionUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_TubeDistortionUI* ui = (gx_TubeDistortionUI*)malloc(sizeof(gx_TubeDistortionUI));
	...
	ui->controls[1] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 120, 40, 61, 61}, false,"INPUT", KNOB, INPUT_};
	ui->controls[2] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 210, 40, 61, 61}, false,"OUTPUT", KNOB, OUTPUT};
	ui->block_event = -1;
...
static void cleanup(LV2UI_Handle handle) {
	gx_TubeDistortionUI* ui = (gx_TubeDistortionUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_TubeDistortionUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_TubeDistortionUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_TubeDistortionUI *ui, gx_controller* controller) {
...
void _expose(gx_TubeDistortionUI *ui) {
	static const char* plug_name = "GxTubeDistortion" ;
...
void controller_expose(gx_TubeDistortionUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
void resize_event(gx_TubeDistortionUI *ui) {
...
static void send_controller_event(gx_TubeDistortionUI *ui, int controller) {
...
static void check_value_changed(gx_TubeDistortionUI *ui, int i, float* value) {
...
static void check_is_active(gx_TubeDistortionUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_TubeDistortionUI *ui) {
...
bool get_active_ctl_num(gx_TubeDistortionUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_TubeDistortionUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
void scroll_event(gx_TubeDistortionUI *ui, int direction) {
	float value;
	int num;
...
static void switch_event(gx_TubeDistortionUI *ui, int i) {
...
void button1_event(gx_TubeDistortionUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
void motion_event(gx_TubeDistortionUI *ui, double start_value, int m_y) {
...
void set_key_value(gx_TubeDistortionUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void key_event(gx_TubeDistortionUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
void set_previous_controller_active(gx_TubeDistortionUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void set_next_controller_active(gx_TubeDistortionUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_TubeDistortionUI *ui, bool set) {
