...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_bottlerocketUI* ui = (gx_bottlerocketUI*)malloc(sizeof(gx_bottlerocketUI));
	...
	ui->controls[4] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 330, 60, 61, 61}, false,"VOLUME", KNOB, VOLUME};
	ui->block_event = -1;
...
static void knob_expose(gx_bottlerocketUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_bottlerocketUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_bottlerocketUI *ui, gx_controller* controller) {
...
void _expose(gx_bottlerocketUI *ui) {
	static const char* plug_name = "GxBottleRocket" ;
...
void controller_expose(gx_bottlerocketUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
bool aligned(int x, int y, gx_controller *control, gx_bottlerocketUI *ui) {
...
void resize_event(gx_bottlerocketUI *ui) {
...
void scroll_event(gx_bottlerocketUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
...
void button1_event(gx_bottlerocketUI *ui, double* start_value) {
	for (int i=0;i<CONTROLS;i++) {
		if (aligned(ui->pos_x, ui->pos_y, &ui->controls[i], ui)) {
...
void set_key_value(gx_bottlerocketUI *ui, int set_value) {
	float value = 0.0;
	for (int i=0;i<CONTROLS;i++) {
...
void key_event(gx_bottlerocketUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
		if (ui->controls[i].is_active) {
			value = min(ui->controls[i].adj.max_value,max(ui->controls[i].adj.min_value, 
...
void set_previous_controller_active(gx_bottlerocketUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
...
void set_next_controller_active(gx_bottlerocketUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
		if(ui->controls[i].is_active == true) {
			ui->controls[i].is_active = false;
			send_controller_event(ui, i);
...
void get_last_active_controller(gx_bottlerocketUI *ui, bool set) {
...
void motion_event(gx_bottlerocketUI *ui, double start_value, int m_y) {
