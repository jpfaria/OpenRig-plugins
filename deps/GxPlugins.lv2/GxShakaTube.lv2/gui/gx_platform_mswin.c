...
void gx_gui_create_window_and_surface(gx_shakatubeUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_shakatubeUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_shakatubeUI *ui) {
	...
	UnregisterClass(TEXT("gx_shakatubeUIClass"), NULL);
}
...
void event_handler(gx_shakatubeUI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_shakatubeUI *ui = (gx_shakatubeUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
