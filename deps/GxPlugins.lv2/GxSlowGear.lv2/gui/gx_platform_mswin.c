...
void gx_gui_create_window_and_surface(gx_slowgearUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_slowgearUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_slowgearUI *ui) {
	...
	UnregisterClass(TEXT("gx_slowgearUIClass"), NULL);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_slowgearUI *ui = (gx_slowgearUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
