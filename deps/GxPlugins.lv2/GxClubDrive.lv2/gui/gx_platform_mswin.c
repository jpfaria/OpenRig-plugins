...
void gx_gui_create_window_and_surface(gx_clubdriveUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_clubdriveUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_clubdriveUI *ui) {
	...
	UnregisterClass(TEXT("gx_clubdriveUIClass"), NULL);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_clubdriveUI *ui = (gx_clubdriveUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
