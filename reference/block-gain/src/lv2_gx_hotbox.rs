...
pub const MODEL_ID: &str = "lv2_gx_hotbox";
pub const DISPLAY_NAME: &str = "Hot Box";
const BRAND: &str = "guitarix";

const PLUGIN_URI: &str =
    "http://guitarix.sourceforge.net/plugins/gx_hotbox_#_hotbox_";
const PLUGIN_DIR: &str = "gx_hotbox";

#[cfg(target_os = "macos")]
const PLUGIN_BINARY: &str = "gx_hotbox.dylib";
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_hotbox.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_hotbox.dll";
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, bass, gain, treble, volume)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, bass, gain, treble, volume)?;
            let right = build_mono_processor(sample_rate, bass, gain, treble, volume)?;
