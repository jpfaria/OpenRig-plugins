...
pub const MODEL_ID: &str = "lv2_gx_knightfuzz";
pub const DISPLAY_NAME: &str = "Knight Fuzz";
...
#[cfg(target_os = "macos")]
const PLUGIN_BINARY: &str = "gx_KnightFuzz.dylib";
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_KnightFuzz.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_KnightFuzz.dll";
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, input, volume)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, input, volume)?;
            let right = build_mono_processor(sample_rate, input, volume)?;
