...
pub const MODEL_ID: &str = "lv2_gx_microamp";
pub const DISPLAY_NAME: &str = "Micro Amp";
...
fn build_mono_processor(sample_rate: f32, gain: f32) -> Result<lv2::Lv2Processor> {
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, gain)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, gain)?;
            let right = build_mono_processor(sample_rate, gain)?;
