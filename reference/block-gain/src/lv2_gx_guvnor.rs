...
pub const MODEL_ID: &str = "lv2_gx_guvnor";
pub const DISPLAY_NAME: &str = "Guvnor";
const BRAND: &str = "guitarix";

const PLUGIN_URI: &str =
    "http://guitarix.sourceforge.net/plugins/gx_guvnor_#_guvnor_";
const PLUGIN_DIR: &str = "gx_guvnor";

#[cfg(target_os = "macos")]
const PLUGIN_BINARY: &str = "gx_guvnor.dylib";
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_guvnor.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_guvnor.dll";
...
const PORT_LEVEL: usize = 4;
const PORT_MID: usize = 5;
const PORT_TREBLE: usize = 6;
...
fn validate_params(params: &ParameterSet) -> Result<()> {
    ...
    let _ = required_f32(params, "mid").map_err(anyhow::Error::msg)?;
...
fn build(
    ...
    let mid = required_f32(params, "mid").map_err(anyhow::Error::msg)? / 100.0;
    let treble = required_f32(params, "treble").map_err(anyhow::Error::msg)? / 100.0;

    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, bass, gain, level, mid, treble)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, bass, gain, level, mid, treble)?;
            let right = build_mono_processor(sample_rate, bass, gain, level, mid, treble)?;
