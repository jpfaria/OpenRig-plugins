...
pub const MODEL_ID: &str = "lv2_gx_suppatonebender";
pub const DISPLAY_NAME: &str = "Suppa Tone Bender";
...
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_vstb.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_vstb.dll";

// LV2 port indices (from TTL)
const PORT_AUDIO_OUT: usize = 0;
const PORT_AUDIO_IN: usize = 1;
const PORT_FUZZ: usize = 2;
...
fn validate_params(params: &ParameterSet) -> Result<()> {
    let _ = required_f32(params, "fuzz").map_err(anyhow::Error::msg)?;
...
fn build(
    ...
    let fuzz = required_f32(params, "fuzz").map_err(anyhow::Error::msg)? / 100.0;
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, fuzz, tone, volume)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, fuzz, tone, volume)?;
            let right = build_mono_processor(sample_rate, fuzz, tone, volume)?;
