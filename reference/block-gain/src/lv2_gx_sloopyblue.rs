...
pub const MODEL_ID: &str = "lv2_gx_sloopyblue";
pub const DISPLAY_NAME: &str = "Sloopy Blue";
const BRAND: &str = "guitarix";

const PLUGIN_URI: &str = "http://guitarix.sourceforge.net/plugins/gx_sloopyblue_#_sloopyblue_";
const PLUGIN_DIR: &str = "gx_sloopyblue";

#[cfg(target_os = "macos")]
const PLUGIN_BINARY: &str = "gx_sloopyblue.dylib";
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_sloopyblue.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_sloopyblue.dll";
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, gain, tone, volume)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, gain, tone, volume)?;
            let right = build_mono_processor(sample_rate, gain, tone, volume)?;
