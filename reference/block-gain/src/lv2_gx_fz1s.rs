...
pub const MODEL_ID: &str = "lv2_gx_fz1s";
pub const DISPLAY_NAME: &str = "Maestro FZ-1S";
const BRAND: &str = "guitarix";

const PLUGIN_URI: &str = "http://guitarix.sourceforge.net/plugins/gx_maestro_fz1s_#_maestro_fz1s_";
const PLUGIN_DIR: &str = "gx_maestro_fz1s";

#[cfg(target_os = "macos")]
const PLUGIN_BINARY: &str = "gx_maestro_fz1s.dylib";
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_maestro_fz1s.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_maestro_fz1s.dll";

// LV2 port indices (from TTL)
const PORT_AUDIO_OUT: usize = 0;
const PORT_AUDIO_IN: usize = 1;
const PORT_BYPASS: usize = 2;
const PORT_BALANCE: usize = 3;
const PORT_FILTER: usize = 4;
const PORT_SUSTAIN: usize = 5;
...
fn build(
    ...
    let filter = required_f32(params, "filter").map_err(anyhow::Error::msg)? / 100.0;
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, balance, filter, sustain, volume)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, balance, filter, sustain, volume)?;
            let right = build_mono_processor(sample_rate, balance, filter, sustain, volume)?;
