...
pub const MODEL_ID: &str = "lv2_gx_timray";
pub const DISPLAY_NAME: &str = "Tim Ray";
const BRAND: &str = "guitarix";

const PLUGIN_URI: &str =
    "http://guitarix.sourceforge.net/plugins/gx_timray_#_timray_";
const PLUGIN_DIR: &str = "gx_timray";

#[cfg(target_os = "macos")]
const PLUGIN_BINARY: &str = "gx_timray.dylib";
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_timray.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_timray.dll";
...
const PORT_BASS: usize = 3;
const PORT_GAIN: usize = 4;
const PORT_TREBLE: usize = 5;
const PORT_TRIM: usize = 6;
const PORT_VOLUME: usize = 7;
...
fn build(
    ...
    let trim = required_f32(params, "trim").map_err(anyhow::Error::msg)? / 100.0;
    let volume = required_f32(params, "volume").map_err(anyhow::Error::msg)? / 100.0;

    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, bass, gain, treble, trim, volume)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, bass, gain, treble, trim, volume)?;
            let right = build_mono_processor(sample_rate, bass, gain, treble, trim, volume)?;
