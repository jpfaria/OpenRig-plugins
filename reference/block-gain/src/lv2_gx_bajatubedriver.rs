...
pub const MODEL_ID: &str = "lv2_gx_bajatubedriver";
pub const DISPLAY_NAME: &str = "BaJa Tube Driver";
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, drive, tone, volume)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, drive, tone, volume)?;
            let right = build_mono_processor(sample_rate, drive, tone, volume)?;
