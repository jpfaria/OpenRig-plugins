...
pub const MODEL_ID: &str = "lv2_gx_creammachine";
pub const DISPLAY_NAME: &str = "Cream Machine";
const BRAND: &str = "guitarix";

const PLUGIN_URI: &str =
    "http://guitarix.sourceforge.net/plugins/gx_CreamMachine_#_CreamMachine_";
const PLUGIN_DIR: &str = "gx_CreamMachine";

#[cfg(target_os = "macos")]
const PLUGIN_BINARY: &str = "gx_CreamMachine.dylib";
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_CreamMachine.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_CreamMachine.dll";
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, bass, level, treble, volume)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, bass, level, treble, volume)?;
            let right = build_mono_processor(sample_rate, bass, level, treble, volume)?;
