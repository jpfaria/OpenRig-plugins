...
pub const MODEL_ID: &str = "lv2_gx_clubdrive";
pub const DISPLAY_NAME: &str = "Club Drive";
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, drive, volume)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, drive, volume)?;
            let right = build_mono_processor(sample_rate, drive, volume)?;
