...
pub const MODEL_ID: &str = "lv2_gx_valvecaster";
pub const DISPLAY_NAME: &str = "Valve Caster";
const BRAND: &str = "guitarix";

const PLUGIN_URI: &str =
    "http://guitarix.sourceforge.net/plugins/gx_valvecaster_#_valvecaster_";
const PLUGIN_DIR: &str = "gx_valvecaster";

#[cfg(target_os = "macos")]
const PLUGIN_BINARY: &str = "gx_valvecaster.dylib";
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_valvecaster.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_valvecaster.dll";
...
fn validate_params(params: &ParameterSet) -> Result<()> {
    ...
    let _ = required_f32(params, "boost").map_err(anyhow::Error::msg)?;
    Ok(())
}
...
fn build(
    ...
    let boost = required_f32(params, "boost").map_err(anyhow::Error::msg)? / 100.0;

    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, gain, tone, volume, boost)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, gain, tone, volume, boost)?;
            let right = build_mono_processor(sample_rate, gain, tone, volume, boost)?;
