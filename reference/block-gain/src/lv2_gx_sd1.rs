...
pub const MODEL_ID: &str = "lv2_gx_sd1";
pub const DISPLAY_NAME: &str = "SD-1";
const BRAND: &str = "guitarix";

const PLUGIN_URI: &str =
    "http://guitarix.sourceforge.net/plugins/gx_sd1sim_#_sd1sim_";
const PLUGIN_DIR: &str = "gx_sd1sim";

#[cfg(target_os = "macos")]
const PLUGIN_BINARY: &str = "gx_sd1sim.dylib";
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_sd1sim.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_sd1sim.dll";
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, drive, level, tone)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, drive, level, tone)?;
            let right = build_mono_processor(sample_rate, drive, level, tone)?;
