...
pub const MODEL_ID: &str = "lv2_gx_dop250";
pub const DISPLAY_NAME: &str = "DOP 250";
const BRAND: &str = "guitarix";

const PLUGIN_URI: &str =
    "http://guitarix.sourceforge.net/plugins/gx_DOP250_#_DOP250_";
const PLUGIN_DIR: &str = "gx_DOP250";

#[cfg(target_os = "macos")]
const PLUGIN_BINARY: &str = "gx_DOP250.dylib";
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_DOP250.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_DOP250.dll";
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, gain, level)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, gain, level)?;
            let right = build_mono_processor(sample_rate, gain, level)?;
