...
pub const MODEL_ID: &str = "lv2_gx_fz1b";
pub const DISPLAY_NAME: &str = "Maestro FZ-1B";
...
#[cfg(target_os = "linux")]
const PLUGIN_BINARY: &str = "gx_maestro_fz1b.so";
#[cfg(target_os = "windows")]
const PLUGIN_BINARY: &str = "gx_maestro_fz1b.dll";
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, balance, volume)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, balance, volume)?;
            let right = build_mono_processor(sample_rate, balance, volume)?;
