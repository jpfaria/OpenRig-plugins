...
pub const MODEL_ID: &str = "lv2_gx_liquiddrive";
pub const DISPLAY_NAME: &str = "Liquid Drive";
...
fn build(
    ...
    match layout {
        AudioChannelLayout::Mono => {
            let processor = build_mono_processor(sample_rate, drive, level)?;
            Ok(BlockProcessor::Mono(Box::new(processor)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono_processor(sample_rate, drive, level)?;
            let right = build_mono_processor(sample_rate, drive, level)?;
