...
/// Body-class silence floor (issue #23). Pickup-emulation body IRs are
/// narrow filters that integrate far below electric cabs against the
/// synthetic DI (−60 to −70 LUFS measured before the #143 level makeup)
/// while still being real captures. −75 flags only dead ones.
...
/// Floor unified at −50 LUFS across classes (issue #23) so heavily cut
/// captures do not read as defects; the SILENCE check at −60 still flags
/// truly dead ones. Too-QUIET-but-alive is `check_level`'s job (#143).
...
pub enum QaFail {
    ...
    LevelOffTarget { peak_dbfs: f32, target_dbfs: f32 },
}
...
impl QaFail {
    pub fn label(&self) -> &'static str {
        match self {
            ...
            QaFail::LevelOffTarget { .. } => "level_off_target",
        }
    }
}

/// Output-level check (issue #143). `post_gain_peak_dbfs` is the block's
/// reference peak (see [`crate::level`]) after the manifest
/// `output_gain_db`. It must sit on `level::TARGET_PEAK_DBFS` within
/// `level::LEVEL_TOLERANCE_DB`: above clips the chain, below wastes
/// headroom and forces the user to crank the Output knob. Below is
/// accepted only when the gain is already pinned at the knob maximum.
pub fn check_level(post_gain_peak_dbfs: f32, applied_gain_db: f32) -> Option<QaFail> {
    use crate::level::{LEVEL_TOLERANCE_DB, OUTPUT_KNOB_MAX_DB, TARGET_PEAK_DBFS};
    let hot = post_gain_peak_dbfs > TARGET_PEAK_DBFS + LEVEL_TOLERANCE_DB;
    let quiet = post_gain_peak_dbfs < TARGET_PEAK_DBFS - LEVEL_TOLERANCE_DB
        && applied_gain_db < OUTPUT_KNOB_MAX_DB;
    (hot || quiet).then_some(QaFail::LevelOffTarget {
        peak_dbfs: post_gain_peak_dbfs,
...
/// `SILENCE_LUFS_BODY` for narrow pickup-emulation body IRs whose
/// convolved LUFS legitimately sits below the standard floor.
...
mod tests {
    ...
    #[test]
    fn level_passes_on_target() {
        assert!(check_level(crate::level::TARGET_PEAK_DBFS, 3.0).is_none());
    }

    #[test]
    fn level_fails_when_headroom_is_wasted() {
        assert!(matches!(
            check_level(-10.0, 5.0),
            Some(QaFail::LevelOffTarget { .. })
        ));
    }

    #[test]
    fn level_accepts_quiet_block_pinned_at_knob_max() {
        assert!(check_level(-10.0, crate::level::OUTPUT_KNOB_MAX_DB).is_none());
    }

    #[test]
    fn level_fails_when_block_runs_hot() {
        assert!(matches!(
            check_level(0.5, -3.0),
            Some(QaFail::LevelOffTarget { .. })
        ));
